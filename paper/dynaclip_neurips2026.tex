...
DynaCLIP does not achieve the highest score on every individual task. DINOv2 leads on T2 (stove + moka pot, 92.0\% vs.\ 89.3\%), VC-1 leads on T3 (bowl in drawer, 97.3\% vs.\ 91.3\%), SigLIP leads on T1 (cream cheese + butter, 57.3\% vs.\ 46.0\%), and CLIP leads on T6 (mug + pudding, 56.0\% vs.\ 46.0\%). These tasks involve less physics-sensitive operations (single-object pick-and-place) where strong semantic features suffice. Importantly, \methodname{} still performs competitively on these tasks ($\le$11pp gap) while dramatically leading on the physics-intensive ones (T0, T5, T7, T8: all best by \methodname{}).
...
\begin{table}[h]
\centering
\caption{\textbf{Summary of \methodname{} results across all experiments.} For each experiment, we report the primary metric, \methodname{}'s value, the best baseline value, and \methodname{}'s rank. \methodname{} achieves \#1 on 13 of 14 metrics.}
\label{tab:summary}
\small
\begin{tabular}{@{}llcccc@{}}
\toprule
